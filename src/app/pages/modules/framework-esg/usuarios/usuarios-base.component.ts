...
export abstract class UsuariosBaseComponent {
    ...
    constructor(public antService: ModFrameworkEsgService, public esgService: FrameworkEsgService) { }
    ...
    init() {
        ...
        this.subsSubmit = this.esgService.onSubmitForm$.subscribe(x => {
            let v = x.cod_bt.value;
            this.usersDataSource.value.push({ cod_bt: v });
            this.submit = true;
        });

        this.esgService.currMetListCat = 1;
    ...
    private loadDs(id:string){
        this.usersDataSource = { value: [] };
        this.metsDataSource = [{ id: '-'+id, name: 'Todos' }].concat(this.esgService.metsLists[id]);
    }
    ...
    addUser() {
        this.esgService.showFormDialog();
    }
    ...
    saveUsers() {
        ...
        this.esgService.openLoader();
        this.antService.postMetUsers(su).subscribe(x=>{
            this.esgService.closeLoader();
        });
    }
