...
@Component({
  ...
  styleUrls: ['./destino-credito.component.scss'],
...
export class DestinoCreditoComponent implements OnInit {
  ...
  public action(event, context) {
    ...
    context.dialog.open(DestinoCreditoPopupComponent, dialogConfig);
  }
  ...
  filter(evt: any) {
    ...
    if (v === "") {
      ...
      this.currentDataSource = this.originalDataSource.filter(x => x.HDESSEC.toLowerCase().includes(v));
    }
    this.prepPagination();
  }
}
