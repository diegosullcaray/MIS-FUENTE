...
@Component({
    selector: 'stg-pinput',
    templateUrl: './stg-pinput.component.html',
    styleUrls: ['./stg-pinput.component.scss'],
    providers: [
        {
            provide: NG_VALUE_ACCESSOR,
            useExisting: forwardRef(() => StgPinputComponent),
            multi: true
        }
    ]
})
export class StgPinputComponent implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    markAsTouched() {
        ...
        if (e.target.value.length) {
            ...
            this.onChange(round(n / 100, this.decimalLength + 2));
    ...
    start(e: any) {
        let n = e.target.value.replace('%', '').replace(',','').trim();
    ...
    writeValue(value: any): void {
        ...
        let sr = '.0-'+this.decimalLength;
        this.textValue = formatNumber(round(value * 100,this.decimalLength), 'en-US', sr) + '%';
    }
