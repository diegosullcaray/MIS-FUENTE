...
import { tableHeaders, loadingConf, tableConf } from './registro-transaccion.util';
import { RegistroTransaccionPopupComponent } from './registro-transaccion-popup/registro-transaccion-popup.component';
...
@Component({
  ...
  styleUrls: ['./registro-transaccion.component.scss'],
  animations: baseAnimations
})
export class RegistroTransaccionComponent implements OnInit {
    matDialogRef: MatDialogRef<RegistroTransaccionPopupComponent>; 
    loadingConf: {};
    tableConf: {};
    name: string = "";
    iconColMap: {};
    ...
    OpenModal() {
        this.matDialogRef = this.dialog.open(RegistroTransaccionPopupComponent, {
          data: { name: this.name },
          disableClose: true
        });
    ...
    public action(event, context) {
      ...
      context.dialog.open(RegistroTransaccionPopupComponent, dialogConfig);
    }
    ...
    page(p: number) {
      ...
      let ss=this.currentDataSource.filter(x => x.pk === p)
